...
TENSOR_STRUCTURAL_CORE_FUNCTIONS = frozenset(
    {
        "arcadia_tio_tensor_to_contiguous",
        "arcadia_tio_tensor_reshape",
        "arcadia_tio_tensor_flatten",
        "arcadia_tio_tensor_expand_dims",
        "arcadia_tio_tensor_squeeze",
        "arcadia_tio_tensor_squeeze_axis",
        "arcadia_tio_tensor_permute_axes",
        "arcadia_tio_tensor_transpose",
        "arcadia_tio_tensor_slice_axis",
        "arcadia_tio_tensor_slice_axis_step",
        "arcadia_tio_tensor_take_axis",
        "arcadia_tio_tensor_index_axis",
    }
)
...
@dataclass(frozen=True)
class FunctionFamilyReport:
    name: str
    expected: int
    present: int
    wrapped: int
    missing_from_headers: tuple[str, ...]
    not_wrapped: tuple[str, ...]

    @property
    def gaps(self) -> int:
        return len(self.missing_from_headers) + len(self.not_wrapped)
...
def function_family_reports(items: Sequence[InventoryItem]) -> list[FunctionFamilyReport]:
    function_items = {item.name: item for item in items if item.kind == "function"}
    reports: list[FunctionFamilyReport] = []
    for family_name, expected_names in NAMED_FUNCTION_FAMILIES.items():
        present = sorted(name for name in expected_names if name in function_items)
        missing_from_headers = tuple(sorted(expected_names.difference(function_items)))
        not_wrapped = tuple(
            name
            for name in present
            if function_items[name].category != CATEGORY_WRAPPED
        )
        wrapped = sum(1 for name in present if function_items[name].category == CATEGORY_WRAPPED)
        reports.append(
            FunctionFamilyReport(
                name=family_name,
...
def print_markdown(items: Sequence[InventoryItem]) -> None:
    ...
    families = function_family_reports(items)
    ...
    print()
    print("A default inventory run fails when `unknown-unmapped` is non-zero. Deferred blockers remain visible and can be promoted to a hard failure with `--fail-on-deferred`. Named family gaps can be promoted to a hard failure with `--fail-on-family-gaps`.")
    print()
    print("## Named family audits")
    print()
    print("| Family | Expected | Present in headers | Wrapped | Gaps |")
    print("| --- | ---: | ---: | ---: | ---: |")
    for family in families:
        print(f"| {family.name} | {family.expected} | {family.present} | {family.wrapped} | {family.gaps} |")
    print()
    for family in families:
        if family.gaps == 0:
            continue
        print(f"### {family.name} gaps")
        print()
        if family.missing_from_headers:
            print("Missing from supplied headers:")
            print()
            for name in family.missing_from_headers:
                print(f"- `{name}`")
            print()
        if family.not_wrapped:
            print("Present but not wrapped:")
            print()
            for name in family.not_wrapped:
                print(f"- `{name}`")
            print()
    for category in [CATEGORY_WRAPPED, CATEGORY_NOT_APPLICABLE, CATEGORY_DEFERRED, CATEGORY_UNKNOWN]:
...
def print_report(items: Sequence[InventoryItem], *, show_all: bool, json_output: bool, markdown_output: bool) -> None:
    ...
    for family in families:
        print(
            f"  - {family.name}: expected {family.expected}, present {family.present}, "
            f"wrapped {family.wrapped}, gaps {family.gaps}"
        )
        if show_all or family.gaps:
            for name in family.missing_from_headers:
                print(f"      missing from supplied headers: {name}")
            for name in family.not_wrapped:
                print(f"      present but not wrapped: {name}")
    print()
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--fail-on-family-gaps", action="store_true", help="also fail when a named C ABI family is missing from headers or not fully wrapped")
...
def main(argv: Sequence[str]) -> int:
    ...
    if counts[CATEGORY_DEFERRED] and args.fail_on_deferred:
        ...
        return 1
    family_gaps = sum(family.gaps for family in function_family_reports(items))
    if family_gaps and args.fail_on_family_gaps:
        print(f"error: {family_gaps} named C ABI family gaps", file=sys.stderr)
        return 1
    return 0
